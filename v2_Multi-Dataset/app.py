...
_DEFAULT_CFG = {
    "data_dir":           "datasets/eicu-collaborative-research-database-demo-2.0",
    "dataset_type":       "eicu",
...
def _worker(job_id: str, config: dict) -> None:
    ...
    try:
        ...
        dataset_type = config.get("dataset_type", "eicu")
        data_dir = os.path.normpath(
            config.get("data_dir", "datasets/eicu-collaborative-research-database-demo-2.0")
        )
        config["dataset_type"] = dataset_type
        push(job, "step", f"Checking data folder: {data_dir}", 3)
        problems = validate_data_dir(data_dir, dataset_type)
        if problems:
            for p in problems:
                push(job, "warn", f"  ✗ {p}")
            push(job, "error",
        ...
        data_count = sum(1 for f in present if f.endswith(".data"))
        push(job, "step",
             f"  ✓ Folder OK [{dataset_type}] — {gz_count} .csv.gz | {csv_count} .csv | {data_count} .data files", 4)
...
def check_path():
    ...
    data_dir    = os.path.normpath(body.get("data_dir", ""))
    ds_type     = body.get("dataset_type", "eicu")
    problems    = validate_data_dir(data_dir, ds_type)
