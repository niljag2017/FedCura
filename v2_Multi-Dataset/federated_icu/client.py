...
from . import config as _cfg  # dynamic FEATURE_COLS for multi-dataset support
...
class FLClient:
    ...
    def __init__(
        ...
        if len(X_raw) > 0:
            self.X: np.ndarray = preprocessor.transform(X_raw[_cfg.FEATURE_COLS])
        else:
            self.X = np.zeros((0, len(_cfg.FEATURE_COLS)))
    ...
    def _skipped_result(self) -> dict:
        return {
            ...
            "local_ap":            0.0,
            "weights":             {},
            "feature_importances": [0.0] * len(_cfg.FEATURE_COLS),
            "skipped":             True,
        }
