...
def validate_data_dir(data_dir: str, dataset_type: str = "eicu") -> List[str]:
    """
    Return a list of problem strings (empty = all OK).

    dataset_type:
      "eicu"  — checks for the 6 required eICU CSV tables (default)
      "heart" — checks for the 4 UCI processed.*.data files
      "bank"  — checks for Churn_Modelling.csv
    """
    problems: List[str] = []
    ...
    if dataset_type == "heart":
        ...
        # Accept any common capitalisation variant
        bank_files = {"churn_modelling.csv", "Churn_Modelling.csv"}
        if not any(f.lower() == "churn_modelling.csv" for f in present):
            problems.append("Missing: Churn_Modelling.csv")

    else:
        # Default: eICU required tables
...
def build_global_test_split(
    ...
    prep = Preprocessor()
    # Re-import FEATURE_COLS dynamically — allows non-eICU datasets to
    # override the feature list without module-level rebind issues.
    from . import config as _cfg_module
    _feature_cols = _cfg_module.FEATURE_COLS
    prep.fit(train_df[_feature_cols])

    X_test = prep.transform(test_df[_feature_cols])
...
def augment_hospital_data(
    ...
    from . import config as _cfg_module
    FEATURE_COLS = _cfg_module.FEATURE_COLS  # always current
